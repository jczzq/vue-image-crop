<template>
  <div class="m-image-crop">
    <img class="show-img" :src="currentValue | fileSrc" />
    <svg
      v-if="hasRemove"
...
export default {
  ...
  methods: {
    imageChange(evt) {
      ...
      this.img.onload = () => {
        ...
        if (this.skipCrop) {
          ...
          if (
            this.config.isSlice &&
            this.filRealitySize.width / this.filRealitySize.height ==
    ...
    loadImg() {
      ...
      let imgHeight =
        imgWidth * (this.filRealitySize.height / this.filRealitySize.width);
      // 解决兼容 ios
    ...
    initViewSize() {
      ...
      // 视图区
      if (
        (this.view.width * this.initViewWidth * this.proportion.h) /
          this.proportion.w <
        ...
        this.viewBox = {
          width: Math.floor(
            (this.view.height * this.initViewWidth * this.proportion.w) /
              this.proportion.h
          ),
    ...
    move(evt) {
      if (this.top) {
        this.fileSize.top = Math.floor(
          this.$refs.file.offsetTop + evt.touches[0].clientY - this.top
        );
        this.fileSize.left = Math.floor(
          this.$refs.file.offsetLeft + evt.touches[0].clientX - this.left
        );
        this.checkPosition();
        this.fileSizePosition();
      }
    ...
    scale(evt) {
      if (this.point1) {
        ...
        this.fileSize.left =
          this.$refs.file.offsetLeft - (newSize.width - oldSize.width) / 2;
        this.fileSize.top =
          this.$refs.file.offsetTop - (newSize.height - oldSize.height) / 2;
        this.checkPosition();
        this.fileSizePosition();
      }
      this.point1 =
    ...
    imgSizeScale() {
      let sizeScale;
      if (
        (this.viewBox.width / this.$refs.file.offsetWidth) *
    ...
    setCanvas() {
      ...
      ctx.drawImage(
        this.$refs.file,
        (this.$refs.file.offsetLeft - this.viewBox.left - this.borderWidth) *
          sizeScale,
        (this.$refs.file.offsetTop - this.viewBox.top - this.borderWidth) *
          sizeScale,
